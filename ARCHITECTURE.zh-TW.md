...
- **月中新人加入 = 整頁 rebuild**(2026-07-28 起):`ensureMonthTab` 讀出當前分頁的 N_old,若 `fullRoster.length > N_old`,呼叫 `rebuildMonthTab`:先 `snapshotMonthTab` 把每個既有日期的 check 明細讀進記憶體(**保留欄位位置**,空格當 0,每個日期算出 sessionCount = 最長列長度,所有玩家補成同長度)→ `values:clear` 清空 values(**格式/欄寬保留**,只清 cell 內容)→ 用新 N 重寫 skeleton → 依原本欄位順序(D→E→F…)逐日 replay 進 rebuilt 分頁。新人在該月之前的每個日期是 sessionCount 個 0(不是空白),對齊其他人;每日總表的 `=SUM(D:AH)` 讀 0 一樣是 0。整個 rebuild 非原子(clear 到 replay 中間掛掉會半空),因此 rebuild 前會 `console.warn` 把 snapshot 內容整包 JSON 印進 log 保底;搭配 Google Sheets 內建的 30 天版本歷史,失敗可以還原。
