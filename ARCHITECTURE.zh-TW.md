...
- **手動維護 CLI**(2026-08-03 起):`server/scripts/drop-user-from-month.ts` 可以在既有月分頁中**物理移除**指定 userId 的列(§11.29 的 `停用` 只影響**未來新建**的分頁,對已存在分頁只是保留 0 列)。走的是相同的 snapshot→clear→rewrite→replay 路徑,加了 zero-sum 安全檢查(要移除的人如果有非 0 值直接拒絕,除非 `--force`)。不加 `--user` 就等於「in-place rebuild」,用來修 format 殘留類問題。呼叫方式:`docker exec -it holdem-server sh -c "cd /app/server && pnpm exec tsx scripts/drop-user-from-month.ts --month YYYY/MM [--user <id>] [--force]"`
